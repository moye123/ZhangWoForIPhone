...
- (void)submit{
    ...
    if ([name length] < 1) {
        [[DSXUI standardUI] showPopViewWithStyle:DSXPopViewStyleError Message:@"请输入收件人姓名"];
        return;
    }
    
    if ([phone length] < 7) {
        [[DSXUI standardUI] showPopViewWithStyle:DSXPopViewStyleError Message:@"请正确输入联系电话"];
        return;
    }
    
    if ([postcode length] != 6) {
        [[DSXUI standardUI] showPopViewWithStyle:DSXPopViewStyleError Message:@"请正确输入邮编"];
        return;
    }
    
    if ([district length] < 1) {
        [[DSXUI standardUI] showPopViewWithStyle:DSXPopViewStyleError Message:@"请选择区域"];
        return;
    }
