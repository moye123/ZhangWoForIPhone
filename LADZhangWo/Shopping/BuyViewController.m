...
- (void)viewDidLoad{
    ...
    NSString *goodsurl;
    UIView *loadingView = [[DSXUI standardUI] showLoadingViewWithMessage:nil];
    ...
    [[AFHTTPSessionManager sharedManager] GET:goodsurl parameters:nil progress:^(NSProgress * _Nonnull downloadProgress) {
        ...
        if ([responseObject isKindOfClass:[NSDictionary class]]) {
            ...
            [[DSXUI standardUI] showPopViewWithStyle:DSXPopViewStyleError Message:@"商品数据读取失败"];
            [self back];
        }
    } failure:^(NSURLSessionDataTask * _Nullable task, NSError * _Nonnull error) {
        [loadingView removeFromSuperview];
        [[DSXUI standardUI] showPopViewWithStyle:DSXPopViewStyleError Message:@"数据加载失败"];
        [self back];
    }];
}
